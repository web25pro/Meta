 ...
 *
 * Both `apps/web` and `apps/admin` mount their own /api/[...path] proxy, so
 * this same relative base URL is correct in either app.
 */
const API_URL = '/api';

/**
 * Where to send the browser when refresh fails. Apps that mount their login
 * page elsewhere can override this at startup via `configureApiClient`.
 */
let loginPath = '/auth/login';

export function configureApiClient(options: { loginPath?: string }): void {
